...
namespace _3DTriangles.Services
{
    ...
    {
        private static List<Vector3> ControlPoints = new();

        public static List<Triangle> GenerateMesh(BezierSurface surface, int resolution)
        {
            // Zapisz punkty kontrolne do późniejszego rysowania
            ControlPoints = surface.GetControlPoints();
        ...
        private static Vertex CreateVertex(BezierSurface surface, float u, float v)
        {
            ...
            var pu = surface.EvaluateDerivativeU(u, v);
            var pv = surface.EvaluateDerivativeV(u, v);
            var n = Vector3.Normalize(Vector3.Cross(pu, pv));
        ...
        public static List<Vector3> GetControlPolygon()
        {
